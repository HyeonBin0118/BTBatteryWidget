...
- 페어링된 블루투스 장치의 배터리 잔량을 5분 주기로 자동 갱신
...
PowerShell이 내부적으로 호출하는 그 API를 우리도 직접 부른다. 결국 `setupapi.dll`의 `SetupDiGetClassDevsW` → `SetupDiEnumDeviceInfo` → `SetupDiGetDevicePropertyW` 체인.
